...
app.get('/admin',auth,async(_req,res)=>{try{const html=await fs.readFile(path.join(__dirname,'admin.html'),'utf8'),version='admin-complete-4',fresh=html.replace('/admin-client-workspace.css?v=8',`/admin-client-workspace.css?v=${version}`).replace('/admin-polish-v2.css?v=dfc7795',`/admin-polish-v2.css?v=${version}`).replace('/admin-client-workspace.js?v=3',`/admin-client-workspace.js?v=${version}`).replace('/admin-category-manager.js?v=6bc8401',`/admin-category-manager.js?v=${version}`);res.set('Cache-Control','no-store');res.type('html').send(fresh)}catch(e){console.error(e);res.status(500).send('Admin page unavailable')}});
