...
function ensureCalculatorUi(){
  if(!document.querySelector('#totalBar')){
    ...
    document.querySelector('#calcItems').addEventListener('click',e=>{
      const control=e.target.closest('[data-calc-action]'); if(!control)return;
      changeQty(control.dataset.id,control.dataset.calcAction==='plus'?1:-1);
    });
  }
}
...
function renderCalculator(){
  ...
  const count=totalCount(); const total=totalPrice();
  bar.hidden=count===0;
...
function card(x){
  ...
  const count=qty(x.id);
  const control=count>0?`<span class="qty-control" aria-label="Количество ${escapeAttr(x.name)}"><span class="qty-btn" data-action="minus" data-id="${x.id}">−</span><b>${count}</b><span class="qty-btn" data-action="plus" data-id="${x.id}">+</span></span>`:`<span class="mini-plus" data-action="plus" data-id="${x.id}" aria-label="Добавить ${escapeAttr(x.name)}">＋</span>`;
  return `<button class="dish-card" type="button" data-dish="${x.id}"><div class="dish-photo">${visual(x)}${badge}</div><div class="dish-body"><h3>${escapeHtml(x.name)}</h3><p>${escapeHtml(x.description||'')}</p><div class="dish-bottom"><strong>${money(x.price)}</strong>${control}</div></div></button>`;
}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){const calc=document.querySelector('#calcSheet');if(calc&&!calc.hidden)closeSheet(calc);else if(!dishSheet.hidden)closeSheet(dishSheet);else if(!infoSheet.hidden)closeSheet(infoSheet);}});
loadMenu();