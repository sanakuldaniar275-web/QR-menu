...
const restaurantFields='id,name,slug,subtitle,phone,address,service,logo_url,hero_image_url,accent_color,theme,background_color,card_style,corner_radius,font_style,hero_height,hero_overlay,category_style,card_columns,active';
...
app.get('/r/:slug',async(_req,res)=>{const html=(await fs.readFile(path.join(__dirname,'index.html'),'utf8')).replace('</head>','<link rel="stylesheet" href="/public-design-studio.css?v=1"></head>');res.type('html').send(html)});
app.get('/admin',auth,async(_req,res)=>{try{const html=await fs.readFile(path.join(__dirname,'admin.html'),'utf8'),version='admin-design-1',fresh=html.replace('/admin-client-workspace.css?v=8',`/admin-client-workspace.css?v=${version}`).replace('/admin-polish-v2.css?v=dfc7795',`/admin-polish-v2.css?v=${version}`).replace('/admin-client-workspace.js?v=3',`/admin-client-workspace.js?v=${version}`).replace('/admin-category-manager.js?v=6bc8401',`/admin-category-manager.js?v=${version}`).replace('</head>',`<link rel="stylesheet" href="/admin-premium.css?v=${version}"><link rel="stylesheet" href="/admin-design-studio.css?v=${version}"></head>`).replace('</body>',`<script src="/admin-premium.js?v=${version}"></script><script src="/admin-design-studio.js?v=${version}"></script></body>`);res.set('Cache-Control','no-store');res.type('html').send(fresh)}catch(e){console.error(e);res.status(500).send('Admin page unavailable')}});
