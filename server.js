...
const restaurantFields='id,name,slug,subtitle,phone,address,service,logo_url,hero_image_url,accent_color,theme,active';
async function menuPayload(rid){const [r,c,d]=await Promise.all([pool.query(`SELECT ${restaurantFields} FROM restaurants WHERE id=$1`,[rid]),pool.query('SELECT id,name,sort_order FROM categories WHERE restaurant_id=$1 ORDER BY sort_order,id',[rid]),pool.query('SELECT id,category_id,name,description,price,image_url,emoji,badge,active,sort_order FROM dishes WHERE restaurant_id=$1 ORDER BY sort_order,id',[rid])]);return {restaurant:r.rows[0],categories:c.rows,dishes:d.rows};}

app.get('/health',async(_req,res)=>{let database='not-configured';if(pool)try{await pool.query('SELECT 1');database='connected';}catch{database='error';}res.set('Cache-Control','no-store').json({ok:true,service:'qr-menu',database,adminConfigured:Boolean(ADMIN_PASSWORD),clientPortal:true});});
...
app.post('/api/admin/restaurants/:id/dishes',auth,requireDb,async(req,res)=>{const {category_id,name,description='',price,image_url='',emoji='',badge=''}=req.body||{};if(!String(name||'').trim()||!Number.isInteger(Number(price))||Number(price)<0)return res.status(400).json({error:'Проверьте название и цену'});const r=await pool.query(`INSERT INTO dishes(restaurant_id,category_id,name,description,price,image_url,emoji,badge,sort_order) VALUES($1,$2,$3,$4,$5,$6,$7,$8,(SELECT COALESCE(MAX(sort_order),-1)+1 FROM dishes WHERE restaurant_id=$1)) RETURNING *`,[req.params.id,category_id||null,String(name).trim(),description,Number(price),image_url,emoji,badge]);res.status(201).json(r.rows[0]);});
app.patch('/api/admin/dishes/:id',auth,requireDb,async(req,res)=>{const {name,description,price,image_url,emoji,badge,active,category_id}=req.body||{},r=await pool.query(`UPDATE dishes SET name=COALESCE($2,name),description=COALESCE($3,description),price=COALESCE($4,price),image_url=COALESCE($5,image_url),emoji=COALESCE($6,emoji),badge=COALESCE($7,badge),active=COALESCE($8,active),category_id=COALESCE($9,category_id) WHERE id=$1 RETURNING *`,[req.params.id,name??null,description??null,price??null,image_url??null,emoji??null,badge??null,active??null,category_id??null]);if(!r.rowCount)return res.status(404).json({error:'Dish not found'});res.json(r.rows[0]);});
...
app.post('/api/client/dishes',requireDb,clientAuth,async(req,res)=>{const {category_id,name,description='',price,image_url='',emoji='',badge=''}=req.body||{};if(!String(name||'').trim()||!Number.isInteger(Number(price))||Number(price)<0)return res.status(400).json({error:'Проверьте название и цену'});if(category_id){const c=await pool.query('SELECT 1 FROM categories WHERE id=$1 AND restaurant_id=$2',[category_id,req.client.restaurant_id]);if(!c.rowCount)return res.status(400).json({error:'Категория не найдена'});}const r=await pool.query(`INSERT INTO dishes(restaurant_id,category_id,name,description,price,image_url,emoji,badge,sort_order) VALUES($1,$2,$3,$4,$5,$6,$7,$8,(SELECT COALESCE(MAX(sort_order),-1)+1 FROM dishes WHERE restaurant_id=$1)) RETURNING *`,[req.client.restaurant_id,category_id||null,String(name).trim(),description,Number(price),image_url,emoji,badge]);res.status(201).json(r.rows[0]);});
app.patch('/api/client/dishes/:id',requireDb,clientAuth,async(req,res)=>{const {name,description,price,image_url,emoji,badge,active,category_id}=req.body||{};if(category_id){const c=await pool.query('SELECT 1 FROM categories WHERE id=$1 AND restaurant_id=$2',[category_id,req.client.restaurant_id]);if(!c.rowCount)return res.status(400).json({error:'Категория не найдена'});}const r=await pool.query(`UPDATE dishes SET name=COALESCE($3,name),description=COALESCE($4,description),price=COALESCE($5,price),image_url=COALESCE($6,image_url),emoji=COALESCE($7,emoji),badge=COALESCE($8,badge),active=COALESCE($9,active),category_id=COALESCE($10,category_id) WHERE id=$1 AND restaurant_id=$2 RETURNING *`,[req.params.id,req.client.restaurant_id,name??null,description??null,price??null,image_url??null,emoji??null,badge??null,active??null,category_id??null]);if(!r.rowCount)return res.status(404).json({error:'Блюдо не найдено'});res.json(r.rows[0]);});
